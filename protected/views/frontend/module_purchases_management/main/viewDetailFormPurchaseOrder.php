...
$this->widget('zii.widgets.CDetailView', array(
   ...
   'attributes'=>array(
      ...
      array(
         'name'=>'payment_method',
         'value'=>(!FString::isNullOrEmpty($oModelForm->payment_method)) ? Yii::t('frontend_' . strtolower(FApplication::MODULE_PURCHASES_MANAGEMENT), 'MODEL_PURCHASESFORMSPURCHASEORDERS_FIELD_PAYMENTMETHOD_VALUE_' . $oModelForm->payment_method) : FString::STRING_EMPTY,
      ),
      array(
         'name'=>'notify_date',
      ...
      array(
         'name'=>'offer',
         'type'=>'raw',
         'value'=>((!FString::isNullOrEmpty($oModelForm->offer)) && (file_exists(FApplication::FOLDER_PROTECTED_DOCUMENTS_MODULE_PURCHASES_OFFERS . $oModelForm->offer))) ? '<a href="' . Yii::app()->controller->createUrl("viewAttachmentOfferFormPurchaseOrder", array("nIdForm"=>$oModelForm->id)) . '">' . Yii::t('system', 'SYS_DOWNLOAD_DOCUMENT') . '</a>' : FString::STRING_EMPTY,
      ),
      array(
         'name'=>'order',
         'type'=>'raw',
         'value'=>((!FString::isNullOrEmpty($oModelForm->order)) && (file_exists(FApplication::FOLDER_PROTECTED_DOCUMENTS_MODULE_PURCHASES_ORDERS . $oModelForm->order))) ? '<a href="' . Yii::app()->controller->createUrl("viewAttachmentOrderFormPurchaseOrder", array("nIdForm"=>$oModelForm->id)) . '">' . Yii::t('system', 'SYS_DOWNLOAD_DOCUMENT') . '</a>' : FString::STRING_EMPTY,
      ),
      array(
         'name'=>'delivery',
         'type'=>'raw',
         'value'=>((!FString::isNullOrEmpty($oModelForm->delivery)) && (file_exists(FApplication::FOLDER_PROTECTED_DOCUMENTS_MODULE_PURCHASES_DELIVERIES . $oModelForm->delivery))) ? '<a href="' . Yii::app()->controller->createUrl("viewAttachmentDeliveryFormPurchaseOrder", array("nIdForm"=>$oModelForm->id)) . '">' . Yii::t('system', 'SYS_DOWNLOAD_DOCUMENT') . '</a>' : FString::STRING_EMPTY,
      ),
      array(
         'name'=>'comments',
      ...
      array(
         'name'=>'changes_reason',
         'value'=>FString::getAbbreviationSentence($oModelForm->changes_reason, 256, 25),
      ),
      array(
