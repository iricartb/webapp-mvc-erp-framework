...
$oDiscartParameters = array('r', 'FormsWorkingParts[sFilterStatusCreated]', 'FormsWorkingParts[sFilterStatusPending]', 'FormsWorkingParts[sFilterStatusRunning]', 'FormsWorkingParts[sFilterStatusPendingAbsence]', 'FormsWorkingParts[sFilterStatusHalted]', 'FormsWorkingParts[sFilterStatusFinalized]');

$sJsParameters = 'function createUrlParameters() {
                     var sFilterStatusCreated = \'&FormsWorkingParts[sFilterStatusCreated]=\';
                     if (document.getElementById(\'id_filter_status_created\').checked) sFilterStatusCreated += \'1\';
                     else sFilterStatusCreated += \'0\';
                     
                     var sFilterStatusPending = \'&FormsWorkingParts[sFilterStatusPending]=\';    
                     if (document.getElementById(\'id_filter_status_pending\').checked) sFilterStatusPending += \'1\';
                     else sFilterStatusPending += \'0\';
                     
                     var sFilterStatusRunning = \'&FormsWorkingParts[sFilterStatusRunning]=\';    
                     if (document.getElementById(\'id_filter_status_running\').checked) sFilterStatusRunning += \'1\';
                     else sFilterStatusRunning += \'0\';
                     
                     var sFilterStatusPendingAbsence = \'&FormsWorkingParts[sFilterStatusPendingAbsence]=\';    
                     ...
                     var sFilterStatusHalted = \'&FormsWorkingParts[sFilterStatusHalted]=\';    
                     if (document.getElementById(\'id_filter_status_halted\').checked) sFilterStatusHalted += \'1\';
                     else sFilterStatusHalted += \'0\';
                     
                     var sFilterStatusFinalized = \'&FormsWorkingParts[sFilterStatusFinalized]=\';    
...
<div class="item-header">
   ...
   <div class="item-header-text">
      <?php echo FString::castStrToUpper(Yii::t('system', 'SYS_MODULE_WORKING_PARTS_MANAGEMENT_HEADER_LINE_FORMS_WORKING_PARTS')); ?>
   </div>
</div>

<div class="item-header-description-padding">
   <?php echo Yii::t('frontend_' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT), 'PAGE_VIEWFORMSWORKINGPARTS_DESCRIPTION'); ?>
</div>
...
<div class="toolbox_table">
   <?php if (!Users::getIsModuleRestricedUser(Yii::app()->user->id, FApplication::MODULE_WORKING_PARTS_MANAGEMENT)) { ?>
      <div class="toolbox_table_first_cell_button_left">
         <?php $sAction = 'window.location = \'' . Yii::app()->controller->createUrl('createFormWorkingPart') . '\''; ?>      
         <?php echo FWidget::showIconImageButton('formsWorkingParts', 'document_new_1.png', Yii::t('frontend_' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT), 'PAGE_VIEWFORMSWORKINGPARTS_NEW_BTN_DESCRIPTION'), $sAction); ?>   
      </div>
   <?php } ?>
   <div class="toolbox_table_first_cell_button">      
      <?php echo FWidget::showToolboxExporterData('formsWorkingParts', FApplication::MODULE_WORKING_PARTS_MANAGEMENT, 'FormsWorkingParts', $oModelForm->sFilterStatusCreated . ',' . $oModelForm->sFilterStatusPending . ',' . $oModelForm->sFilterStatusRunning . ',' . $oModelForm->sFilterStatusPendingAbsence . ',' . $oModelForm->sFilterStatusHalted . ',' . $oModelForm->sFilterStatusFinalized, FString::STRING_EMPTY, $this, true); ?>   
   </div>
</div>
...
<div class="row">
   ...
   if ((is_null($oWorkingPartModuleParameters)) || ((!is_null($oWorkingPartModuleParameters)) && ($oWorkingPartModuleParameters->working_part_show_status_created)) || ((!is_null($oWorkingPartModuleParameters)) && ($oWorkingPartModuleParameters->working_part_show_status_created == false) && ($oWorkingPartModuleParameters->working_part_show_status_pending == false) && ($oWorkingPartModuleParameters->working_part_show_status_running == false) && ($oWorkingPartModuleParameters->working_part_show_status_pending_absence == false) && ($oWorkingPartModuleParameters->working_part_show_status_halted == false) && ($oWorkingPartModuleParameters->working_part_show_status_finalized == false))) { ?>
      <div class="cell">
         <input id="id_filter_status_created" type="checkbox" onclick="<?php echo $sJsParameters . 'window.location.href=\'' . $this->createUrl('frontend/' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT) . '/main/viewFormsWorkingParts') . FUrl::getUrlParameters($oDiscartParameters) . '\' + createUrlParameters()'?>" <?php echo $sFilterStatusCreated;?>>
         <input type="text" style="height:10px; width:25px; border:1px solid transparent; background-color:<?php echo FModuleWorkingPartsManagement::COLOR_GRID_STATUS_CREATED;?>; cursor:default;" readonly/>&nbsp<?php echo Yii::t('frontend_' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT), 'MODEL_FORMSWORKINGPARTS_FIELD_STATUS_VALUE_CREATED');?>
      </div>
   <?php
   } else { ?>
   ...
   if ((!is_null($oWorkingPartModuleParameters)) && ($oWorkingPartModuleParameters->working_part_show_status_pending)) { ?>  
      <div class="cell">
         <input id="id_filter_status_pending" type="checkbox" onclick="<?php echo $sJsParameters . 'window.location.href=\'' . $this->createUrl('frontend/' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT) . '/main/viewFormsWorkingParts') . FUrl::getUrlParameters($oDiscartParameters) . '\' + createUrlParameters()'?>" <?php echo $sFilterStatusPending;?>>
         <input type="text" style="height:10px; width:25px; border:1px solid transparent; background-color:<?php echo FModuleWorkingPartsManagement::COLOR_GRID_STATUS_PENDING;?>; cursor:default;" readonly/>&nbsp<?php echo Yii::t('frontend_' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT), 'MODEL_FORMSWORKINGPARTS_FIELD_STATUS_VALUE_PENDING');?>
      </div>
   <?php
   } else { ?>
   ...
   if ((!is_null($oWorkingPartModuleParameters)) && ($oWorkingPartModuleParameters->working_part_show_status_running)) { ?> 
      <div class="cell">
         <input id="id_filter_status_running" type="checkbox" onclick="<?php echo $sJsParameters . 'window.location.href=\'' . $this->createUrl('frontend/' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT) . '/main/viewFormsWorkingParts') . FUrl::getUrlParameters($oDiscartParameters) . '\' + createUrlParameters()'?>" <?php echo $sFilterStatusRunning;?>>
         <input type="text" style="height:10px; width:25px; border:1px solid transparent; background-color:<?php echo FModuleWorkingPartsManagement::COLOR_GRID_STATUS_RUNNING;?>; cursor:default;" readonly/>&nbsp<?php echo Yii::t('frontend_' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT), 'MODEL_FORMSWORKINGPARTS_FIELD_STATUS_VALUE_RUNNING');?>
      </div>
   <?php
   } else { ?>
   ...
   if ((!is_null($oWorkingPartModuleParameters)) && ($oWorkingPartModuleParameters->working_part_show_status_pending_absence)) { ?>  
      <div class="cell">
         <input id="id_filter_status_pending_absence" type="checkbox" onclick="<?php echo $sJsParameters . 'window.location.href=\'' . $this->createUrl('frontend/' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT) . '/main/viewFormsWorkingParts') . FUrl::getUrlParameters($oDiscartParameters) . '\' + createUrlParameters()'?>" <?php echo $sFilterStatusPendingAbsence;?>>
         <input type="text" style="height:10px; width:25px; border:1px solid transparent; background-color:<?php echo FModuleWorkingPartsManagement::COLOR_GRID_STATUS_PENDING_ABSENCE;?>; cursor:default;" readonly/>&nbsp<?php echo Yii::t('frontend_' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT), 'MODEL_FORMSWORKINGPARTS_FIELD_STATUS_VALUE_PENDING_ABSENCE');?>
      </div>
   <?php
   } else { ?>
   ...
   if ((!is_null($oWorkingPartModuleParameters)) && ($oWorkingPartModuleParameters->working_part_show_status_halted)) { ?> 
      <div class="cell">
         <input id="id_filter_status_halted" type="checkbox" onclick="<?php echo $sJsParameters . 'window.location.href=\'' . $this->createUrl('frontend/' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT) . '/main/viewFormsWorkingParts') . FUrl::getUrlParameters($oDiscartParameters) . '\' + createUrlParameters()'?>" <?php echo $sFilterStatusHalted;?>>
         <input type="text" style="height:10px; width:25px; border:1px solid transparent; background-color:<?php echo FModuleWorkingPartsManagement::COLOR_GRID_STATUS_HALTED;?>; cursor:default;" readonly/>&nbsp<?php echo Yii::t('frontend_' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT), 'MODEL_FORMSWORKINGPARTS_FIELD_STATUS_VALUE_HALTED');?>
      </div>
   <?php
   } else { ?>
   ...
   if ((!is_null($oWorkingPartModuleParameters)) && ($oWorkingPartModuleParameters->working_part_show_status_finalized)) { ?> 
      <div class="last_cell">
         <input id="id_filter_status_finalized" type="checkbox" onclick="<?php echo $sJsParameters . 'window.location.href=\'' . $this->createUrl('frontend/' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT) . '/main/viewFormsWorkingParts') . FUrl::getUrlParameters($oDiscartParameters) . '\' + createUrlParameters()'?>" <?php echo $sFilterStatusFinalized;?>>
         <input type="text" style="height:10px; width:25px; border:1px solid transparent; background-color:<?php echo FModuleWorkingPartsManagement::COLOR_GRID_STATUS_FINALIZED;?>; cursor:default;" readonly/>&nbsp<?php echo Yii::t('frontend_' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT), 'MODEL_FORMSWORKINGPARTS_FIELD_STATUS_VALUE_FINALIZED');?>
      </div>
   <?php
   } else { ?>
