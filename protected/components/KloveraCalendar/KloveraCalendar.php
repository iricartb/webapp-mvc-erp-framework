...
*/
	...
	*/
		...
		var $weekstartday = 1; 					// week start day (0-6 e.g. 0 = Sunday, 1 = Monday, etc.)
		var $jqueryinclude = true;			   // includes jquery from google (true,false)
		var $monthselector = false;			// month/year select box (true=show selector,false=show month name)
      ...
      var $fulllinkbase = '';             // base url for links
      var $showbuttons = false;           // show buttons of header calendar
      var $infodetails = 'Details';
      var $close = 'close';
	...
	//--------------------------------------------------------------------------------------------
		function CALENDAR($type='full',$date='',$staticdisplaytype=false) { // types: mini,full,list,mini-list (default=full)
			...
			if($staticdisplaytype===true) {
               ...
               if (($_REQUEST['t'] != 'mini') && ($_REQUEST['t'] != 'full') && ($_REQUEST['t'] != 'list') && ($_REQUEST['t'] != 'mini-list')) $this->displaytype = $type; 
					else $this->displaytype = $_REQUEST['t']; // else set to type passed in querystring
				}
			}
      ...
      function setMonthName() {
				...
				$(function() {
					...
					$("table#cal_'.$cal_ID.'.full td.has-events").click(function() {
						...
						if($(this).find("table#cal_'.$cal_ID.' .open-details").length<=0) {
							...
							details.html("<a href=\"#\" class=\"close-details\">' . $this->close . '</a>"+contents);
		...
		function calendar_head($content='') {
			...
			if(!in_array($this->displaytype,array("list","mini-list"))) { // mini and full cal
				...
				if($this->displaytype == 'full' && $this->monthselector === true) {
					...
					$html .= '<span id="month_current">'.$this->monthname.' '.$this->year.'</span>';
				}
				$html .= $this->cal_next(); // next month link
				if($this->displaytype!='mini') 
				if ($this->showbuttons) $html .= $this->cal_viewmode(); // viewmode buttons
				$html .= '	
						</div>
					</td>';
				...
				if($this->displaytype=='list') {// don't show viewmode buttons for mini-list
					...
					if ($this->showbuttons) $html .= $this->cal_viewmode();
				}
			}
		...
		function cal_next() {
         ...
         if (strpos($this->fulllinkbase, '?') !== false) {
            $html = '<div class="next"><a href="'.$this->fulllinkbase.'&date='.$next['year'].'-'.$next['month'].'">'.$nexttext.'</a></div>';
         }
         else if (strlen($this->fulllinkbase) > 0) {
            $html = '<div class="next"><a href="'.$this->fulllinkbase.'?date='.$next['year'].'-'.$next['month'].'">'.$nexttext.'</a></div>'; 
         } 
         else $html = '<div class="next"><a href="?date='.$next['year'].'-'.$next['month'].'&t='.$this->displaytype.'">'.$nexttext.'</a></div>'; 
		...
		function cal_previous() {
         ...
         if (strpos($this->fulllinkbase, '?') !== false) {
			   $html = '<div class="previous"><a href="'.$this->fulllinkbase.'&date='.$previous['year'].'-'.$previous['month'].'">'.$nexttext.'</a></div>';
			}
         else if (strlen($this->fulllinkbase) > 0) {
            $html = '<div class="previous"><a href="'.$this->fulllinkbase.'?date='.$previous['year'].'-'.$previous['month'].'">'.$nexttext.'</a></div>'; 
         } 
         else $html = '<div class="previous"><a href="?date='.$previous['year'].'-'.$previous['month'].'&t='.$this->displaytype.'">'.$nexttext.'</a></div>'; 
		...
		function calendar_cell($day, $class, $date = '', $style = '') {
			...
			if(count($cellevents) > 0) {
				...
				foreach($cellevents as $cellevent) {
					...
					if($cellevent['details']!='') $eventcontent .= '<div><span class="event-metalabel">' . $this->infodetails . ':</span><br/>'.$cellevent['details'].'</div>';
					$eventcontent .= '</div>';
					$eventcontent .= '</li>';
					}
		...
		function getMonth($date,$type=1) {
         ...
         return $this->months[$date_parts[1] - 1];	
		}
