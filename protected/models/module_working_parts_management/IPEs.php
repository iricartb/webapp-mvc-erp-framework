...
/**
 * This is the model class for table "ipes".
 *
 * The followings are the available columns in table 'ipes':
 * @property integer $id
 * @property string $name
 */
class IPEs extends CExportedActiveRecord {
	...
	public function relations() {
		...
		return array(
         'risks'=>array(self::HAS_MANY, 'RisksIpes', 'id_ipe'),
      );
	}
	...
	public function attributeLabels() {
		return array(
			...
			'name'=>Yii::t('frontend_' . strtolower(FApplication::MODULE_WORKING_PARTS_MANAGEMENT), 'MODEL_IPES_FIELD_NAME'),
		);
	}
   ...
   public static function getIPE($nId) {
      $oIPE = IPEs::model()->findByPk($nId);
      
      return $oIPE;
   }
   
   public static function getIPEName($nId) {
      $oIPE = IPEs::getIPE($nId);
      if (!is_null($oIPE)) {
         return $oIPE->name;      
      }  
   ...
   public static function getIPEs() {
      return IPEs::model()->findAll(array('order'=>'name ASC'));      
   }
